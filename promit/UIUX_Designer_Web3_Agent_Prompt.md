...
2. 页面风格
   ...
   - 整体偏向web3风格，以简洁为主
   - **Web3核心特性要求**：所有界面必须包含钱包连接状态显示、网络切换功能、区块链交易状态提示（待确认/处理中/成功/失败）、Gas费显示、交易哈希链接等Web3原生元素；支持多语言布局（中英文）；遵循Web3交互规范（钱包连接流程、交易确认界面、错误处理机制）；确保设计规范文档和用户流程图完整输出，作为下游开发Agent的权威依据。
