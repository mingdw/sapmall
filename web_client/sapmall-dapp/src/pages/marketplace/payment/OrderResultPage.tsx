...
import { formatSap } from '../product/utils/priceDisplay';
import PaymentGlassCard from './components/PaymentGlassCard';
import OrderResultStepIndicator from './components/OrderResultStepIndicator';
...
const OrderResultPage: React.FC = () => {
  ...
  return (
    <div className={`mx-auto w-[95%] max-w-2xl py-10 md:py-14 ${styles.pageShell}`}>
      <div className={styles.pageGlow} aria-hidden />
      <div className={styles.pageInner}>
        <Breadcrumb
          className={`mb-6 text-sm ${styles.breadcrumb}`}
          items={[
            {
              title: (
                <Link to="/marketplace" className={styles.breadcrumbLink}>
                  {t('payment.breadcrumb.mall')}
                </Link>
              ),
            },
            {
              title: (
                <span className={styles.breadcrumbCurrent}>{t('payment.result.breadcrumb')}</span>
              ),
            },
          ]}
        />

        <OrderResultStepIndicator />
                  ...
                  {state.amount != null ? (
                    <>
                      <div className="flex justify-between">
                        ...
                        <dd className="text-orange-400 font-semibold">{formatUsdcDisplay(state.amount)}</dd>
                      </div>
                      <div className="flex justify-between">
                        <dt className="text-slate-400">{t('payment.result.paidSap')}</dt>
                        <dd className="text-slate-300">{formatSap(state.amount)}</dd>
                      </div>
                    </>
                  ) : null}
                  {state.chainId ? (
