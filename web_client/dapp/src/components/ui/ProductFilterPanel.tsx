...
export const ProductFilterPanel: React.FC = () => {
  ...
  return (
    <div className="mb-6">
      <div className={styles['filter-card'] + ' rounded-xl p-4'}>
        {/* 商品目录筛选 */}
        <div className="mb-2">
            ...
            <div className="flex flex-wrap gap-2 items-start flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-2 items-center flex-1">
                {categories.map(cat => (
                  <div
                    key={cat.key}
                    className={styles['category-item'] + ` category-item px-3 py-2 rounded-lg font-medium cursor-pointer${selectedCategory === cat.key ? ' active' : ''}`}
                  ...
                  >
                    <div className="flex items-center justify-between w-full">
                      ...
                      <span className={styles['category-count'] + ' category-count rounded-full'}>
                        <span>{cat.count}</span>
                      </span>
                    </div>
                    {/* 下拉子分类 */}
              ...
              <div className="flex gap-2 items-center">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="搜索商品..."
                    className={styles['search-input'] + " search-input w-72 pl-9 pr-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:border-sapphire-500 focus:outline-none focus:bg-gray-700 transition-all text-sm"}
                    value={search}
          ...
          <div className="space-y-3 relative">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <div className="flex flex-wrap gap-2">
                  {priceFilters.map((item, idx) => (
                    <button
                      key={idx}
                      className={styles['filter-tag'] + ` filter-tag px-2.5 py-1 rounded-md text-xs${selectedPrice === item.label.zh ? ' bg-sapphire-600 text-white' : ''}`}
                  ...
                  {selectedPrice && (
                    <button className={styles['clear-single-btn'] + " clear-single-btn px-2.5 py-1 rounded-md text-xs"} onClick={() => setSelectedPrice(null)}>
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <div className="flex flex-wrap gap-2">
                  {ratingFilters.map((item, idx) => (
                    <button
                      key={idx}
                      className={styles['filter-tag'] + ` filter-tag px-2.5 py-1 rounded-md text-xs${selectedRating === item ? ' bg-sapphire-600 text-white' : ''}`}
                  ...
                  {selectedRating && (
                    <button className={styles['clear-single-btn'] + " clear-single-btn px-2.5 py-1 rounded-md text-xs"} onClick={() => setSelectedRating(null)}>
                      <i className="fas fa-times mr-1"></i>清除
                    </button>
                  )}
                </div>
            ...
            <div className="flex flex-wrap gap-2 items-center justify-between">
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <div className="flex flex-wrap gap-2">
                  {salesFilters.map((item, idx) => (
                    <button
                      key={idx}
                      className={styles['filter-tag'] + ` filter-tag px-2.5 py-1 rounded-md text-xs${selectedSales === item.label.zh ? ' bg-sapphire-600 text-white' : ''}`}
                  ...
                  {selectedSales && (
                    <button className={styles['clear-single-btn'] + " clear-single-btn px-2.5 py-1 rounded-md text-xs"} onClick={() => setSelectedSales(null)}>
                      <i className="fas fa-times mr-1"></i>清除
                    </button>
                  )}
                </div>
              ...
              <div className="flex gap-2">
                <button className={styles['clear-btn'] + " clear-btn px-3 py-1.5 rounded-lg font-medium text-xs"} onClick={() => {
                  setSelectedPrice(null); setSelectedRating(null); setSelectedSales(null);
                }}>
                  <i className="fas fa-trash mr-1"></i>清空筛选
                </button>
                <button className={styles['more-btn'] + " more-btn px-3 py-1.5 rounded-lg font-medium text-xs"} onClick={() => setExpanded(v => !v)}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 items-center justify-between">
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>最近一周</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>最近一月</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>最近三月</button>
                    </div>
                  </div>
                </div>
                {/* 类型 */}
                <div className="flex flex-wrap gap-2 items-center justify-between">
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>数字商品</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>服务类</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>订阅制</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>一次性购买</button>
                    </div>
                  </div>
                </div>
                {/* 商家信誉 */}
                <div className="flex flex-wrap gap-2 items-center justify-between">
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>官方认证</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>高信誉商家</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>新商家</button>
                    </div>
                  </div>
                </div>
                {/* 特色标签 */}
                <div className="flex flex-wrap gap-2 items-center justify-between">
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>限时折扣</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>免费试用</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>开源项目</button>
                      <button className={styles['filter-tag'] + " filter-tag px-2.5 py-1 rounded-md text-xs"}>独家发布</button>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button className={styles['clear-btn'] + " clear-btn px-3 py-1.5 rounded-lg font-medium text-xs"} onClick={() => {
                    ...
                    <button className={styles['more-btn'] + " more-btn px-3 py-1.5 rounded-lg font-medium text-xs"} onClick={() => setExpanded(false)}>
