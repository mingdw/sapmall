import type { TFunction } from 'i18next';
import type {
  SecurityScore,
  WalletSecuritySettings,
  AccessControlSettings,
  DeviceInfo,
  ActivityRecord,
  WhitelistAddress,
  SecurityData,
  ActivityFilterType,
} from './types';

export type {
  SecurityScore,
  WalletSecuritySettings,
  AccessControlSettings,
  DeviceInfo,
  ActivityRecord,
  WhitelistAddress,
  SecurityData,
  ActivityFilterType,
};

export const ACTIVITY_PAGE_SIZE = 6;

const ACTIVITY_FILTER_DEFS: { key: ActivityFilterType; labelKey: string }[] = [
  { key: 'all', labelKey: 'personal.security.filterAll' },
  { key: 'login', labelKey: 'personal.security.filterLogin' },
  { key: 'transaction', labelKey: 'personal.security.filterTransaction' },
  { key: 'settings', labelKey: 'personal.security.filterSettings' },
];

/** @deprecated 请使用 getActivityFilterOptions(t) */
export const ACTIVITY_FILTER_OPTIONS: { key: ActivityFilterType; label: string }[] =
  ACTIVITY_FILTER_DEFS.map((d) => ({ key: d.key, label: d.labelKey }));

export function getActivityFilterOptions(t: TFunction) {
  return ACTIVITY_FILTER_DEFS.map((d) => ({ key: d.key, label: t(d.labelKey) }));
}

export const mockSecurityData: SecurityData = {
  score: {
    score: 65,
    level: 'medium',
  },
  walletSecurity: {
    transactionConfirmation: true,
    highValueAlert: true,
    contractInteractionWarning: true,
  },
  accessControl: {
    twoFactorEnabled: false,
    autoLock: true,
    addressWhitelist: false,
  },
  devices: [
    {
      id: 'device-1',
      name: 'Windows Chrome',
      icon: 'fa-desktop',
      lastActive: '2024-06-01 14:30:22',
      ip: '192.168.1.1',
      isCurrent: true,
      platform: 'Windows',
    },
    {
      id: 'device-2',
      name: 'iPhone Safari',
      icon: 'fa-mobile-alt',
      lastActive: '2024-05-28 09:15:43',
      ip: '172.16.0.5',
      isCurrent: false,
      platform: 'iOS',
    },
    {
      id: 'device-3',
      name: 'macOS Firefox',
      icon: 'fa-laptop',
      lastActive: '2024-05-25 18:22:10',
      ip: '10.0.0.8',
      isCurrent: false,
      platform: 'macOS',
    },
  ],
  activities: [
    {
      id: 'act-1',
      type: 'login',
      title: 'personal.security.activityMock.loginSuccess.title',
      time: '2024-06-01 14:30:22',
      detail: 'personal.security.activityMock.loginSuccess.detail',
      status: 'success',
      icon: 'fa-sign-in-alt',
    },
    {
      id: 'act-2',
      type: 'transaction',
      title: 'personal.security.activityMock.purchaseProduct.title',
      time: '2024-05-31 18:45:10',
      detail: 'personal.security.activityMock.purchaseProduct.detail',
      status: 'success',
      icon: 'fa-exchange-alt',
    },
    {
      id: 'act-3',
      type: 'settings',
      title: 'personal.security.activityMock.securityChange.title',
      time: '2024-05-30 10:15:33',
      detail: 'personal.security.activityMock.securityChange.detail',
      status: 'success',
      icon: 'fa-cog',
    },
    {
      id: 'act-4',
      type: 'login',
      title: 'personal.security.activityMock.loginFailed.title',
      time: '2024-05-29 22:10:45',
      detail: 'personal.security.activityMock.loginFailed.detail',
      status: 'warning',
      icon: 'fa-exclamation-triangle',
      warning: true,
    },
    {
      id: 'act-5',
      type: 'transaction',
      title: 'personal.security.activityMock.deposit.title',
      time: '2024-05-28 09:45:12',
      detail: 'personal.security.activityMock.deposit.detail',
      status: 'success',
      icon: 'fa-wallet',
    },
    {
      id: 'act-6',
      type: 'settings',
      title: 'personal.security.activityMock.kycSubmit.title',
      time: '2024-05-27 16:30:20',
      detail: 'personal.security.activityMock.kycSubmit.detail',
      status: 'pending',
      icon: 'fa-user-shield',
    },
  ],
  whitelistAddresses: [
    { address: '0x1234567890abcdef1234567890abcdef12345678', addedAt: '2024-05-20 10:30' },
    { address: '0x9876543210fedcba9876543210fedcba98765432', addedAt: '2024-05-18 14:25' },
  ],
};
