...
**2. 三层用户角色权限系统**
- **普通用户**: 钱包连接即可使用基础功能，参与质押和治理
- **认证用户**: 通过KYC认证后可发布商品，查看销售数据
- **管理员**: 平台指定管理人员，负责审核和平台维护
...
| 分配对象 | 数量 | 比例 | 锁仓期 | 释放机制 |
|----------|------|------|--------|----------|
| **公开众筹** | 30,000,000 SAP | 30% | 无 | 众筹结束后立即流通 |
| **团队激励** | 20,000,000 SAP | 20% | 12个月 | 线性释放24个月 |
| **生态发展** | 25,000,000 SAP | 25% | 6个月 | 根据生态需要释放 |
| **流动性挖矿** | 20,000,000 SAP | 20% | 无 | 根据挖矿算法释放 |
| **储备资金** | 5,000,000 SAP | 5% | 18个月 | 应急和发展储备 |
...
**奖励计算公式**:
```
用户奖励 = (用户LP代币数量 / 总LP代币数量) × 池子每日奖励 × 天数
```
