...
3.  **成为陪审员**: 用户需锁定一定数量的SAP代币，才能获得成为纠纷处理陪审员的资格。
4.  **商家保证金**: 认证商家需要锁定SAP作为信誉保证金，保证服务质量。
...
-   **价值捕获**:
    ...
    -   所有需要锁定SAP的场景（如商家保证金、陪审员资格）都会减少市场的流通供应量。
-   **通缩机制**:
...
- Maria完成商家认证，并锁定了少量SAP作为保证金。
- 她上传了自己的设计模板，填写了中英文描述和价格（以USDC计价）。
- 社区审核员（由锁定SAP的活跃用户担任）快速审核通过了她的商品。
...
- **功能逻辑**: 用户在个人中心提交认证申请，可能需要提供社交媒体链接、过往作品集等证明。社区或平台团队审核通过后，用户需锁定指定数量的SAP作为保证金，完成认证。
...
- **功能逻辑**: 买卖双方可发起仲裁申请。系统从锁定了SAP的陪审员池中随机选择成员组成陪审团。陪审团根据双方证据进行投票，系统根据结果自动执行判决（如退款、赔付保证金），并奖励陪审员。
- **验收标准**: 仲裁流程高效、透明；判决结果能被强制执行。

**交互要求**:
- 明确的陪审员工作台，展示待处理案件与历史记录
- 贡献任务的领取、提交与审核流程清晰可见
- 奖励发放状态实时反馈
- 社区讨论与提案入口易于访问
...
- **个人中心入口**：
  - 钱包地址右侧的圆形用户头像图标
  - 点击显示下拉菜单：资产概览、交易历史、治理中心、设置
  - 或直接跳转到个人中心页面

#### 6.3.3 资产与支付页面设计

**资产概览界面**:
- **代币余额卡片**：显示 SAP、稳定币等可用余额
- **优惠权益**：展示可用的支付折扣、积分、优惠券
- **订单提醒**：列出待支付、待确认的近期订单

**支付记录界面**:
- **记录列表**：按时间展示支付/退款记录及状态
- **筛选条件**：支持按币种、状态、时间过滤
- **快捷操作**：可直接跳转至订单详情、下载支付凭证
